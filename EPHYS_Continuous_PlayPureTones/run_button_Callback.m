...
TCam = 1000/FsCam; % frame duration
TCam = TCam*1.35;  % make TCam larger to be sure that at least one frame is in that duration
RP.SetTagVal('TCam', TCam);
...
for trial = 1:Ntrials
    ...
    for tone = 1:NTones
        ...
        disp([num2str(f/1000), ' kHz, ',...
            ...
            num2str(tone+(trial-1)*NTones),'/',num2str(tot_trials)])
        fprintf('\n');
        t1(tone) = toc; % This is true time order
    end
end
