...
clc
RP = TDTRP('Ephus_PWFMSweeps_GUI.rcx', 'RX6');
...
Ntrials(isnan(Ntrials)) = 30; % if the box is empty then this is default value.
...
[F, FreqOrder, SweepDirOrder, AmpOrder, TimeOrder] = FDATorder(Fspec, Amp_Array,...
	ISI, jitter, Ntrials, Randomized, UpDown, FsCam, nFrames, NCam); 

SweepTimeOrder = Pulse_Dur*ones(size(FreqOrder));
SweepRateOrder = 1000*BW_Oct./SweepTimeOrder;
% play out the sounds
...
for nc = 1:NCam
    ...
    for n = 1:length(idx)
        ...
        freq = FOrdertemp(n);
        % get frequency range
        min_freq = freq/(2^(BW_Oct/2));
        max_freq = freq*(2^(BW_Oct/2));
        [PWFMSweepG, N] = FMchirp(fs,min_freq,max_freq,Sdir,Pulse_Dur,ramp);

        % apply calibration filter and adjust amplitudes for required dB SPL
        Amp = AOrdertemp(n);
        sigRMS = rms(PWFMSweepG);
        sigVolts = sigRMS*sqrt(2);
        a = 10^((Amp-calib_level)/20)*calib_V;
        PWFMSweepG = PWFMSweepG*a/sigVolts;
        PWFMSweepGC = conv(PWFMSweepG,FIRcoeff,'same');
        
        % load up entire buffer
        RP.WriteTagVEX('datain', 0, 'F32', PWFMSweepGC);
        RP.SetTagVal('PulseSamp', N);
        RP.SetTagVal('SourceSize', N);
        Rate = SROrdertemp(n);
        disp(sprintf('%s/%s. PWFM Sweep: %s oct/s; %sward; %skHz - %skHz; %s dB SPL',...
...
filename = ['0',num2str(fileno),'_PWS', '_fmin_',num2str(min_Freq),...
	'_fmax_',num2str(max_Freq), '_FreqPerOct_',num2str(Freq_per_Oct),'_BWoct_',num2str(BW_Oct),...
    '_Amp_',Amp_str,...
	'dur_',num2str(Pulse_Dur), '_ramp_',num2str(ramp),...
