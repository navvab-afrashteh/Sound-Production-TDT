...
if isempty(Amp_Array)
	Amp_Array = 10:10:90; % if the box is empty then this is default value.
end
...
%% Generate click
[C,ClickOrder,AmpClick,PhaseClick] = ClickAPorder(Amp_Array,Nclick,Randomized);
...
if Click
    ...
    StimTDT = bufferStim(ClickOrder,AmpTDT_Click,PhaseClick,click,n1,n2,idx1,idx2);
...
n2=0;idx2=0;
[n1,n2,idx1,idx2] = bufferInfo(n2,idx2,npts,N_ISI);
StimTDT = bufferStim(FreqIndex,AmpTDT_Tone,PhaseTone,tone,n1,n2,idx1,idx2);
TDT.write('datain', StimTDT);
index = length(StimTDT);
if ~Click % if clicks have played trigger has already arrived
    % waiting for trigger from other hardware
    fprintf('TDT is waiting for the trigger from A-M system\n')
    while ~TDT.read('Trigger')
    end
